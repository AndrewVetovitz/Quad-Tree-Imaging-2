...
class QuadTreeNode implements IQuadTreeNode {
  ...
  constructor(startx: number, starty: number, endx: number, endy: number, imageData: ImageData, rowWidth: number) {
    this.startx = startx;
    this.starty = starty;
    this.endx = endx;
    this.endy = endy;
    const data = calculateRGBAndWeightedError(imageData, startx, starty, endx, endy, rowWidth);
