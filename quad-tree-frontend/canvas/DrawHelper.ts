import { QuadTreeNode } from "../components/QuadTree/QuadTreeNode";
...
class DrawHelper {
  ...
  constructor(context: CanvasRenderingContext2D, imageData: ImageData, canvasWidth: number) {
  ...
  drawBorder(startx: number, starty: number, endx: number, endy: number, colorEnum: Color) {
  ...
  updateNodes(nodes: Array<QuadTreeNode>) {
    for (let i = 0; i < nodes.length; i++) {
      ...
      this.drawBorder(nodes[i].startx, nodes[i].starty, nodes[i].endx, nodes[i].endy, Color.BLACK);
    }
  }
}
